...
int
gy_mlkem512_keypair(uint8_t *pk, uint8_t *sk)
{
    if (pk == NULL || sk == NULL)
        return GY_ERR_ARG;
...
int
gy_mlkem512_encaps(uint8_t *ct, uint8_t *ss, const uint8_t *pk)
{
...
int
gy_mlkem512_decaps(uint8_t *ss, const uint8_t *ct, const uint8_t *sk)
{
