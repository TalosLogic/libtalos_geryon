...
/*
 * The direct OQS_SIG_ml_dsa_87_* entry points are stateless and take no OQS_SIG
...
int
gy_mldsa87_keypair(uint8_t *pk, uint8_t *sk)
{
    if (pk == NULL || sk == NULL)
        return GY_ERR_ARG;

    if (OQS_SIG_ml_dsa_87_keypair(pk, sk) != OQS_SUCCESS) {
        gy_secure_zero(sk, GY_MLDSA87_SK);
...
int
gy_mldsa87_sign(uint8_t *sig, const uint8_t *sk, const uint8_t *msg,
                size_t mlen, const uint8_t *ctx, size_t ctxlen)
{
    ...
    if (OQS_SIG_ml_dsa_87_sign_with_ctx_str(sig, &siglen, msg, mlen, ctx,
                                            ctxlen, sk) != OQS_SUCCESS) {
        gy_secure_zero(sig, GY_MLDSA87_SIG);
        return GY_ERR_CRYPTO;
    }
    /* ML-DSA signatures are fixed length; a short write is a provider fault. */
    if (siglen != GY_MLDSA87_SIG) {
        gy_secure_zero(sig, GY_MLDSA87_SIG);
        return GY_ERR_CRYPTO;
    }
    return GY_OK;
...
int
gy_mldsa87_verify(const uint8_t *sig, const uint8_t *pk, const uint8_t *msg,
                  size_t mlen, const uint8_t *ctx, size_t ctxlen)
{
    if (pk == NULL || sig == NULL)
        return GY_ERR_ARG;
    ...
    if (OQS_SIG_ml_dsa_87_verify_with_ctx_str(msg, mlen, sig, GY_MLDSA87_SIG,
