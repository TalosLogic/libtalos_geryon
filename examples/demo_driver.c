 ...
 * deadlock).
 *
 * This body is suite-agnostic: demo_run() takes the GY_SUITE_* value and the
 * per-run temp base from its thin-main caller (main.c / hybrid_main.c) and
 * threads the suite into each client's config.  Nothing else changes between
 * the classical and hybrid demos.
 */

#define _POSIX_C_SOURCE 200809L
/*
 * macOS guards mkdtemp() behind the FULL Darwin namespace level; defining
 * _POSIX_C_SOURCE alone lowers the level to POSIX-only and hides it. This
 * restores the full level on Darwin and is ignored by glibc (which exposes
 * mkdtemp via _POSIX_C_SOURCE >= 200809L).
 */
...
/*
 * Per-run temp base and per-client store directory paths.  The dir buffers are
 * strictly larger than the base plus the longest "/<name>" suffix so the
 * snprintf below cannot truncate (base is a runtime-sized buffer, so the
 * compiler bounds the concatenation by these sizes).
 */
#define DEMO_BASE_MAX 256
#define DEMO_DIR_MAX (DEMO_BASE_MAX + 16)
static char g_alice_dir[DEMO_DIR_MAX];
static char g_bob_dir[DEMO_DIR_MAX];
...
int
demo_run(uint8_t suite, const char *base_template)
{
    struct sigaction sa;
    char base[DEMO_BASE_MAX];
    ...
    if (snprintf(base, sizeof(base), "%s", base_template) >=
        (int)sizeof(base)) {
        fprintf(stderr, "demo: base template too long\n");
        return 1;
    }
    ...
    if (g_alice_pid == 0) {
        struct client_cfg cfg = {"alice",          "bob",
                                 g_alice_dir,      "alice-secret",
                                 CLIENT_INITIATOR, suite};
    ...
    if (g_bob_pid == 0) {
        struct client_cfg cfg = {"bob",        "alice",          g_bob_dir,
                                 "bob-secret", CLIENT_RESPONDER, suite};
