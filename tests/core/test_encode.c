...
TEST(pkid_kats)
{
    ...
    /* c448 (SHA-512 tier) computes a pkid over the 57-byte encoding. */
    enc[0] = GY_CURVE_TYPE_448;
...
TEST(info_kats)
{
    ...
    /* c448 is enabled and carries its own suite name. */
    ASSERT_EQ(gy_info(out, sizeof(out), &outlen, GY_SUITE_C448, "x3dh"), GY_OK);
