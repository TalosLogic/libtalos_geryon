 ...
 *
 * Tests for src/core/mldsa44.c (ML-DSA-44, FIPS 204), built with
 * -DGY_TEST_HOOKS so the pqinit RNG draw-count hook is live.
 *
...
TEST(roundtrip)
{
    ...
    ASSERT_EQ(gy_mldsa44_keypair(pk, sk), GY_OK);
    ASSERT_EQ(gy_mldsa44_sign(sig, sk, msg, sizeof(msg), ctx, sizeof(ctx) - 1),
              GY_OK);
    ASSERT_EQ(
        gy_mldsa44_verify(sig, pk, msg, sizeof(msg), ctx, sizeof(ctx) - 1),
        GY_OK);

    memcpy(bad, msg, sizeof(msg));
    bad[0] ^= 0x01;
    ASSERT_EQ(
        gy_mldsa44_verify(sig, pk, bad, sizeof(bad), ctx, sizeof(ctx) - 1),
        GY_ERR_VERIFY);
}
...
TEST(keypair_invariants)
{
    ...
    ASSERT_EQ(gy_mldsa44_keypair(pk1, sk1), GY_OK);
    ASSERT_EQ(gy_mldsa44_keypair(pk2, sk2), GY_OK);
    ...
    ASSERT_EQ(gy_mldsa44_keypair(NULL, sk1), GY_ERR_ARG);
    ASSERT_EQ(gy_mldsa44_keypair(pk1, NULL), GY_ERR_ARG);
}

/*
 * The context is load-bearing (D-PQ-1): a signature made under one ctx must not
 * verify under a different ctx, and empty vs nonempty ctx must not
 * cross-verify.
 */
TEST(ctx_negative)
{
    ...
    ASSERT_EQ(
        gy_mldsa44_sign(sig, sk, msg, sizeof(msg), ctx_a, sizeof(ctx_a) - 1),
        GY_OK);
    ASSERT_EQ(
        gy_mldsa44_verify(sig, pk, msg, sizeof(msg), ctx_a, sizeof(ctx_a) - 1),
        GY_OK);
    ASSERT_EQ(
        gy_mldsa44_verify(sig, pk, msg, sizeof(msg), ctx_b, sizeof(ctx_b) - 1),
        GY_ERR_VERIFY);
    ASSERT_EQ(gy_mldsa44_verify(sig, pk, msg, sizeof(msg), NULL, 0),
              GY_ERR_VERIFY);

    /* Empty ctx: verifies empty, fails under a nonempty ctx. */
    ASSERT_EQ(gy_mldsa44_sign(sig, sk, msg, sizeof(msg), NULL, 0), GY_OK);
    ASSERT_EQ(gy_mldsa44_verify(sig, pk, msg, sizeof(msg), NULL, 0), GY_OK);
    ASSERT_EQ(
        gy_mldsa44_verify(sig, pk, msg, sizeof(msg), ctx_a, sizeof(ctx_a) - 1),
        GY_ERR_VERIFY);
    ...
    /* ctxlen over the FIPS 204 bound is rejected before the provider. */
    {
        ...
        ASSERT_EQ(gy_mldsa44_sign(sig, sk, msg, sizeof(msg), big, sizeof(big)),
                  GY_ERR_TOOLONG);
    }
}
...
TEST(hedged_smoke)
{
    ...
    ASSERT_EQ(gy_mldsa44_keypair(pk, sk), GY_OK);
    ASSERT_EQ(gy_mldsa44_sign(sig1, sk, msg, sizeof(msg), ctx, sizeof(ctx) - 1),
              GY_OK);
    ASSERT_EQ(gy_mldsa44_sign(sig2, sk, msg, sizeof(msg), ctx, sizeof(ctx) - 1),
              GY_OK);
    ASSERT_TRUE(memcmp(sig1, sig2, GY_MLDSA44_SIG) != 0,
                "hedged signatures differ");
    ASSERT_EQ(
        gy_mldsa44_verify(sig1, pk, msg, sizeof(msg), ctx, sizeof(ctx) - 1),
        GY_OK);
    ASSERT_EQ(
        gy_mldsa44_verify(sig2, pk, msg, sizeof(msg), ctx, sizeof(ctx) - 1),
        GY_OK);
}
...
TEST(kat_sigver_valid)
{
    ...
    ASSERT_EQ(gy_mldsa44_verify(sig, pk, msg, GY_DSA_VER_MSG_LEN, ctx,
                                GY_DSA_VER_CTX_LEN),
              GY_OK);
}
...
TEST(kat_sigver_reject)
{
    ...
    ASSERT_EQ(gy_mldsa44_verify(sig, pk, msg, GY_DSA_REJ_MSG_LEN, ctx,
                                GY_DSA_REJ_CTX_LEN),
              GY_ERR_VERIFY);
}
